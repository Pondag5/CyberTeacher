...
| **40** | **`/sandbox`** | **Песочница для кода** (Docker) ✨ |
...
## 🎯 Высокоприоритетные фичи (H-01..H-09)

### H-01: Визуальная карта сети (`/network`)
ASCII-графика текущего состояния лабораторий:
- Отображает все хосты (DVWA, Juice Shop и др.) с их статусом (работает/остановлен)
- Цветовая индикация (зелёный/красный)
- Считает запущенные контейнеры

### H-02: Система RAM — экипировка инструментов (`/tools`, `/equip`)
Ограниченный набор инструментов, которые можно экипировать перед заданием:
- Каждый инструмент имеет «стоимость» RAM (nmap=20, sqlmap=30, burpsuite=25 и т.д.)
- Максимум 100 RAM
- Команды: `/tools` — список, `/equip <tool>` — добавить/удалить
- Выбор сохраняется между сессиями

### H-03: Таймер Trace (`time_limit_minutes`)
Для лабораторий можно задать ограничение по времени:
- По истечении — уведомление и подсказка
- Пример: DVWA запускается с 30-минутным лимитом
- Управление через состояние (`/health` показывает таймер)

### H-04: Единая сюжетная кампания (`/story`, `/flag`, `/achievements`)
20 последовательных эпизодов по веб-уязвимостям:
- Каждый эпизод имеет цель, подсказку, флаг и лабораторию
- Прогресс сохраняется, опыт начисляется
- Достижения и уровни: Script Kiddie → Master Hacker

### H-05: Редактор миссий (`/missions`, `/mission start`, `/mission submit`)
Пользовательские миссии в формате JSON:
- Создайте файл в папке `missions/` (пример: `intro_web.json`)
- Структура: id, title, description, difficulty, steps (objective, hint, flag, lab), xp_reward, time_limit_minutes
- `/missions` — список, `/mission start <id>` — начать, `/mission submit <id>` — завершить (автоматическая проверка флагов)
- Миссии сохраняются в прогресс

### H-06: Интеграция CVE (`/cve <CVE-ID>`)
Поиск информации об уязвимости из NVD (National Vulnerability Database):
- Автоматический запрос к NVD REST API
- Получается описание, уровень CVSS, ссылки
- Кэширование на 1 час
- Пример: `/cve CVE-2024-1234`

### H-07: Сканирование GitHub/GitLab на секреты (`/scan <repo_url> [branch]`)
Анализ репозитория на наличие секретных ключей, токенов, паролей в истории:
- Клонирует репозиторий (используя git)
- Сканирует все файлы по шаблонам: AWS keys, Slack, GitHub tokens, OpenAI, generic password assignments
- Выводит первыe 50 находок с контекстом
- Временные файлы удаляются

### H-08: Метрики и health (`/health`)
Команда показывает таблицу метрик:
- Аптайм, количество LLM-запросов, среднее время ответа, токенов
- Статистика кэша (хиты/промахи, %)
- Текущий режим
- Rate limiting: запросов за последнюю минуту

### H-09: Веб-интерфейс (Streamlit)
Мониторинговая панель в браузере:
- Запуск: `streamlit run web_ui.py`
- Вкладки: Обзор (метрики), Сеть (ASCII), Лаборатории (управление Docker), Сканер CVE, Миссии
- Реальное время (чтение `memory/app_state.json`)
