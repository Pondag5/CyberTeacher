...
if not DOCKER_AVAILABLE:
    print("[yellow]⚠️ Docker не найден в системе. Практические лаборатории будут недоступны.[/yellow]")
...
DOCKER_LABS = {
    ...
    "metasploitable2": {
        ...
        "ports": {"2222": "22", "4444": "4444", "8000": "8000", "8080": "80", "21": "21", "23": "23"},
...
def get_container_status(name: str) -> Dict:
    ...
    code, stdout, _ = run_docker_cmd(["ps", "-a", "--filter", f"name={name}", "--format", "{{.Status}}"])
...
def get_all_running_labs() -> Dict[str, Dict]:
    ...
    result = {}
    for lab_key in DOCKER_LABS.keys():
...
def start_lab(lab_name: str) -> str:
    ...
    if "db" in lab:
        ...
        if not status["running"]:
            db_image = lab["db"]
            run_docker_cmd(["run", "-d", "--name", db_name, "--network", net_name, 
                          "-e", "MYSQL_ROOT_PASSWORD=root", "-e", "MYSQL_DATABASE=dvwa", db_image])
    ...
    run_docker_cmd(["run", "-d", "--name", web_name, "--network", net_name] + 
                   port_args + env_args + [lab["image"]])
...
def list_labs() -> str:
    ...
    cat_order = ["web", "sqli", "api", "jwt", "linux", "windows", "network", "mobile", "cloud", "ctf", "api"]
...
def list_practices() -> str:
    ...
    for cat in categories:
        ...
        result += f"📂 {cat.upper()}:\n"
        for lab in labs:
            result += f"  #{lab.id} {lab.name} {PracticeHub.DIFFICULTY_MAP[lab.difficulty]}\n"
        result += "\n"
...
HTB_MACHINES = [
    {"name": "Blue", "os": "Windows", "difficulty": "easy", "skills": ["SMB", "MS17-010"]},
    {"name": "Legacy", "os": "Windows", "difficulty": "easy", "skills": ["SMB", "Windows"]},
