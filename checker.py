...
def audit_knowledge_base():
    ...
    files = [
        f for f in os.listdir(KNOWLEDGE_DIR) if f.endswith((".txt", ".md", ".pdf"))
    ]
    results = []
    ...
    for filename in files:
        ...
        if size < 100:
            results.append(
                f"❌ [red]{filename}[/red] - Пустой или слишком маленький ({size} байт)"
            )
            continue
        ...
        if filename.endswith(".txt") or filename.endswith(".md"):
            try:
                with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    if is_error:
                        results.append(
                            f"❌ [red]{filename}[/red] - Похоже на ошибку HTML/404"
                        )
                    else:
                        # Если все ок, показываем начало для уверенности
                        snippet = start_content[:50].replace("\n", " ")
                        results.append(
                            f"✅ [green]{filename}[/green] ({size // 1024}KB) - '{snippet}...'"
                        )
            ...
            results.append(
                f"✅ [green]{filename}[/green] ({size // 1024}KB) - Бинарный файл (PDF)"
            )
