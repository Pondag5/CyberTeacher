...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        c.execute("INSERT INTO stats (points, last_activity) VALUES (0, ?)", (datetime.now().isoformat(),))
...
def get_chat_history(conn, limit: int = 10):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT role, content, mode FROM messages ORDER BY id DESC LIMIT ?",
        (limit,)
    )
    rows = c.fetchall()
    return [{"role": row[0], "content": row[1], "mode": row[2]} for row in reversed(rows)]
...
def update_topic_progress(conn, topic: str, is_correct: bool):
    ...
    if row:
        ...
        new_total = row[1] + 1
        c.execute("UPDATE progress SET correct = ?, total = ?, last_seen = ? WHERE topic = ?",
                  (new_correct, new_total, now, topic))
    else:
        c.execute("INSERT INTO progress (topic, correct, total, last_seen) VALUES (?, ?, ?, ?)",
