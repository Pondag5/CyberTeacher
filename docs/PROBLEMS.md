...
### A-01: Неполное использование state.py (ЧАСТИЧНО)
...
**Статус:** ⚠️ In progress (основное переведено, остались следы)
...
### UX-01: Нет справки по новым командам
**Описание:** `/help` не показывает `/social`, `/risk` и другие новые команды.

**Решение:** Обновить `ui.py` show_help() с полным списком.

**Файлы:** `ui.py`

**Приоритет:** Medium

**Статус:** ⚠️ In progress (частично)
