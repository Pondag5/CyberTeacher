...
def _simulate_shodan_search(query: str) -> list[dict[str, Any]]:
    ...
    for _ in range(count):
        results.append({
            ...
            "vulns": random.sample(["CVE-2021-44228", "CVE-2022-22965", "CVE-2023-44487"], k=random.randint(0, 2)),
        })
    return results
...
def _simulate_shodan_host(ip: str) -> dict[str, Any]:
    ...
    return {
        "ip": ip,
        "ports": random.sample([22, 80, 443, 8080, 3306, 5432, 6379, 27017, 9200], k=random.randint(2, 5)),
        "os": random.choice(["Linux 4.x", "Windows Server 2019", "Ubuntu 22.04"]),
        "hostname": f"server-{random.randint(1,999)}.example.com",
        "org": random.choice(["Amazon", "Google Cloud", "Microsoft Azure", "DigitalOcean"]),
...
def _display_shodan_search(query: str) -> bool:
    ...
    if SHODAN_AVAILABLE and api_key:
        try:
            ...
            console.print(f"[green]Найдено {results['total']} результатов (реальный API).[/green]")
            return True
    ...
    for r in results:
        ...
        table.add_row(r["ip"], str(r["port"]), r["product"] + vuln_str, r["os"], r["country"])
