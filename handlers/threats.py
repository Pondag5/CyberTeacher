    ...
    try:
        ...
        if len(parts) > 1:
            ...
            if not os.path.exists(threat_file):
                ...
                # Список доступных
                files = (
                    [f[:-5] for f in os.listdir(THREATS_DIR) if f.endswith(".json")]
                    if os.path.exists(THREATS_DIR)
                    else []
                )
                if files:
                    console.print(
                        "[cyan]Доступные: " + ", ".join(sorted(files)) + "[/cyan]"
                    )
...
def handle_threat_summary(
    ...
    try:
        ...
        if not threat_news:
            console.print(
                "[yellow]Нет свежих новостей об угрозах за последний час.[/yellow]"
            )
        ...
        if llm is None:
            ...
            console.print(
                Panel(
                    "\n".join([n["title"] for n in threat_news[:8]]),
                    title="Свежие угрозы",
                )
            )
            return True, None, None, True
