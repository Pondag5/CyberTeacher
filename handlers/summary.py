...
def handle_summary(action: str) -> Tuple[bool, Optional[Any], Optional[Any], bool]:
    ...
    try:
        ...
        try:
            ...
            context = "\n\n".join([f"Источник {i+1}:\n{doc.page_content[:2000]}" for i, doc in enumerate(docs[:5])])
...
"""
            ...
            console.print(Panel(summary, title=f"Конспект: {topic}", border_style="cyan", expand=True))
            
            console.print("\n[yellow]Сохранить конспект в файл? (y/n)[/yellow]")
            try:
                save = input("> ").strip().lower()
                if save in ('y', 'yes', 'д', 'да'):
                    filename = f"summary_{topic.replace(' ', '_')}_{int(time.time())}.md"
