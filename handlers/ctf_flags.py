...
def handle_ctf_flags(action: str) -> HandlerResult:
...
def _generate(challenge_id: str) -> HandlerResult:
...
def _list_challenges() -> HandlerResult:
    ...
    for cid, name, diff, xp in challenges:
        ...
        lines.append(
            f"  [cyan]{cid:<20}[/cyan] [{diff_color}]{diff:<8}[/] {name} [dim]({xp} XP)[/dim]"
        )
