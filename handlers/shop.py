...
def load_shop_items() -> list[dict[Any, Any]]:
...
def get_dynamic_price(item: dict[Any, Any], reputation: int) -> int:
...
def handle_shop(action: str) -> HandlerResult:
    ...
    if subcmd == "history":
        ...
        if not history:
            ...
            for entry in history[-15:]:
                dt = datetime.fromtimestamp(entry.get("timestamp", 0)).strftime(
                    "%Y-%m-%d %H:%M"
                )
    ...
    # Покупка предмета
    items = load_shop_items()
    selected_item: dict[Any, Any] | None = next(
        (i for i in items if i["id"] == subcmd), None
    )
    ...
    actual_cost = get_dynamic_price(selected_item, state.reputation)
    if state.points < actual_cost:
        console.print(
            f"[red]Недостаточно XP. Нужно {actual_cost}, у вас {state.points}[/red]"
        )
        return True, None, None, True

    # Проверка, если это тема — уже ли владеет?
    if selected_item["type"] == "theme":
        theme_value = selected_item.get("value")
        if theme_value and theme_value in state.owned_themes:
            console.print(
                f"[yellow]Вы уже владеете тему '{selected_item['name']}'[/yellow]"
            )
            return True, None, None, True
    # Для topic_unlock можно проверять, уже ли разблокирован
    if selected_item["type"] == "unlock_topic":
        topic_value = selected_item.get("value")
    ...
    state.apply_item_effect(selected_item)
    ...
    state.purchase_history.append(
        {
            ...
            "cost": actual_cost,
            "base_cost": selected_item["cost"],
        }
    )

    ctx.save_state()

    console.print(f"[green]✅ Куплено: {selected_item['name']}![/green]")
    if actual_cost < selected_item["cost"]:
        saved = selected_item["cost"] - actual_cost
    ...
    elif selected_item["type"] == "unlock_topic":
        console.print(
            f"Тема '{selected_item.get('value')}' добавлена в доступные для quiz/adaptive"
        )
    elif selected_item["type"] == "consumable":
        console.print(
            f"Добавлено: {selected_item.get('quantity', 1)} x {selected_item['name']}"
        )
    elif selected_item["type"] == "xp_boost":
