...
from handlers.theme import THEMES, get_theme_colors, handle_theme
...
class TestThemeHandler(unittest.TestCase):
    ...
    def test_list_themes(self, mock_console, mock_get_context):
        ...
        mock_state.current_theme = "ocean"
        mock_ctx = MagicMock()
    ...
    def test_set_valid_theme(self, mock_console, mock_get_context):
        ...
        success, _, _, continue_loop = handle_theme("/theme sunset")

        self.assertTrue(success)
        self.assertEqual(mock_state.current_theme, "sunset")
    ...
    @patch("handlers.theme.get_context")
    def test_get_theme_colors_ocean(self, mock_get_context):
        mock_state = MagicMock()
        mock_state.current_theme = "ocean"
        mock_ctx = MagicMock()
        mock_ctx.state = mock_state
    ...
    @patch("handlers.theme.get_context")
    def test_get_theme_colors_matrix(self, mock_get_context):
        mock_state = MagicMock()
        mock_state.current_theme = "matrix"
        mock_ctx = MagicMock()
        mock_ctx.state = mock_state
        mock_get_context.return_value = mock_ctx

        colors = get_theme_colors()
        self.assertEqual(colors["border"], "bright_green")
    ...
    def test_set_theme_initializes_if_missing(self, mock_console, mock_get_context):
        ...
        self.assertEqual(mock_state.current_theme, "ocean")
