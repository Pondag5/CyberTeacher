...
class TestOSINTSearch(unittest.TestCase):
    ...
    def test_valid_username(self):
        ...
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_search("testuser")
            self.assertTrue(action_taken)

    def test_short_username(self):
        """Поиск с коротким никнеймом (<3 символов)."""
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_search("ab")
            self.assertTrue(action_taken)

    def test_empty_username(self):
        """Поиск с пустым никнеймом."""
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_search("")
            self.assertTrue(action_taken)
...
class TestOSINTEmail(unittest.TestCase):
    ...
    def test_valid_email(self):
        ...
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_email("test@example.com")
            self.assertTrue(action_taken)

    def test_invalid_email(self):
        """Проверка невалидного email."""
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_email("invalid-email")
            self.assertTrue(action_taken)
...
class TestOSINTPhone(unittest.TestCase):
    ...
    def test_valid_phone(self):
        ...
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_phone("+79991234567")
            self.assertTrue(action_taken)

    def test_short_phone(self):
        """Поиск по короткому номеру."""
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_phone("123")
            self.assertTrue(action_taken)
...
class TestOSINTMetadata(unittest.TestCase):
    ...
    def test_valid_file(self):
        ...
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_metadata("test.jpg")
            self.assertTrue(action_taken)

    def test_empty_file(self):
        """Анализ без указания файла."""
        with patch("handlers.osint.console.print"):
            _, result, _, action_taken = handle_osint_metadata("")
            self.assertTrue(action_taken)
