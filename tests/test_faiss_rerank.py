...
class TestFAISSReranking(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.documents = [
            Document(
                page_content="Cybersecurity is the practice of protecting systems and networks from digital attacks.",
                metadata={"source": "doc1", "topic": "security"},
            ),
            Document(
                page_content="Intrusion Detection System (IDS) monitors network traffic for suspicious activity.",
                metadata={"source": "doc2", "topic": "ids"},
            ),
            Document(
                page_content="Data encryption transforms information into an unreadable format to protect confidentiality.",
                metadata={"source": "doc3", "topic": "encryption"},
            ),
            Document(
                page_content="Multi-factor authentication requires multiple proofs of identity for access.",
                metadata={"source": "doc4", "topic": "auth"},
            ),
            Document(
                page_content="Phishing attacks use deceptive emails to steal credentials.",
                metadata={"source": "doc5", "topic": "phishing"},
            ),
            Document(
                page_content="Firewall controls incoming and outgoing network traffic based on security rules.",
                metadata={"source": "doc6", "topic": "firewall"},
            ),
            Document(
                page_content="Vulnerability assessment helps identify weaknesses in security systems.",
                metadata={"source": "doc7", "topic": "vulnerability"},
            ),
            Document(
                page_content="SOC (Security Operations Center) is a team that monitors threats and incidents.",
                metadata={"source": "doc8", "topic": "soc"},
            ),
            Document(
                page_content="Penetration testing (pentest) simulates attacks to find security holes.",
                metadata={"source": "doc9", "topic": "pentest"},
            ),
            Document(
                page_content="Zero-day vulnerabilities are unknown to vendors and have no patches.",
    ...
    def test_relevant_docs_returns_correct_number(self):
        ...
        self.assertEqual(
            len(relevant_docs), k, f"Expected {k} documents, got {len(relevant_docs)}"
        )
