...
from handlers.threats import THREATS_DIR, handle_groups, handle_threats
...
class TestThreatsCommands(unittest.TestCase):
    ...
    def test_handle_threats_missing_group(
        self, mock_listdir, mock_exists, mock_console
    ):
    ...
    @patch("handlers.threats.os.listdir")
    @patch("builtins.open", create=True)
    def test_handle_groups_shows_summary(
        self, mock_open, mock_listdir, mock_exists, mock_console
    ):
    ...
    def test_actual_json_files_exist(self):
        ...
        self.assertGreaterEqual(
            len(files),
            5,
            f"Нужно минимум 5 JSON-файлов для C-04, найдено: {len(files)}",
        )
